...
const initialAdmins = [
  ...
  },
  {
    ...
    departmentId: "20",
    phone: "9876543210",
    address: "456 Silicon Valley, Mumbai",
  },
];
      ...
      <div className="text-center">
        ...
        <div className="bg-gray-100 border border-gray-200 rounded-lg p-3 my-4">
          <label className="text-sm font-semibold text-gray-500">
            {success ? "Admin ID (Autogenerated)" : "Error Code"}
          </label>
          <p className="text-lg font-mono text-gray-800 mt-1">
            {success ? id : errorCode}
          </p>
        </div>
        <button
          onClick={onClose}
          className={`w-full px-6 py-3 font-bold text-white rounded-lg shadow-md transition ${
            success
    ...
    <Modal isOpen={isOpen} onClose={onClose} size="sm">
      <div className="text-center">
        ...
        <p className="text-gray-600 mb-6">
          ...
          <span className="font-semibold">{adminName}</span>? This action cannot
          be undone.
        </p>
...
const AdminForm = ({ initialData, onSave, onCancel, isEdit = false }) => {
  ...
  const validate = () => {
    ...
    else if (!/^\d+$/.test(formData.departmentId))
      newErrors.departmentId = "Department ID must be a number.";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <h3 className="text-xl font-bold text-center text-gray-800 mb-4">
        {isEdit ? "Edit Admin Details" : "Enter New Admin Details"}
      </h3>
      ...
      <InputField
        ...
        onChange={handleChange}
        placeholder="Admin Address here...."
    ...
    <Modal isOpen={isOpen} onClose={onClose} size="sm">
      <div className="text-center">
        ...
        <button
          ...
          className="w-full mt-4 px-6 py-3 bg-green-600 text-white font-bold rounded-lg shadow-md hover:bg-green-700 transition"
        >
          OK
        </button>
      </div>
...
const AdminDetails = () => {
  ...
  const handleUpdateAdmin = (updatedData) => {
    setAdmins((prev) =>
      prev.map((admin) =>
        admin.id === modalState.data.id ? { ...admin, ...updatedData } : admin
      )
    );
  ...
  const handleConfirmDelete = () => {
    setAdmins((prev) =>
      prev.filter((admin) => admin.id !== modalState.data.id)
    );
    setModalState({ type: null, data: null });
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          <div className="flex justify-end gap-4 mb-6">
            <button
              onClick={() => setModalState({ type: "editPrompt" })}
            ...
            <button
              onClick={() => setModalState({ type: "addForm" })}
              className="flex items-center gap-2 px-5 py-2 bg-[#0d214f] text-white font-semibold rounded-lg shadow-md hover:bg-opacity-90 transition"
            >
      ...
      {/* Modals */}
      <Modal
        isOpen={modalState.type === "addForm"}
      ...
      <EditPromptModal
        isOpen={modalState.type === "editPrompt"}
        onClose={() => setModalState({ type: null })}
        onConfirm={handleEditPrompt}
      />

      <Modal
        isOpen={modalState.type === "editForm"}
        onClose={() => setModalState({ type: null })}
      >
        <AdminForm
          isEdit={true}
